...
export async function sendContactNotification(
  ...
  const recipient = notification.recipient || process.env.CONTACT_TO_EMAIL || "gokulakannan7972@gmail.com";

  if (!apiKey) {
    return { sent: false } as const;
  }

  const safeName = escapeHtml(notification.name);
  const safeEmail = escapeHtml(notification.email);
  const safeCompany = notification.company ? escapeHtml(notification.company) : "Not specified";
  const safeMessage = escapeHtml(notification.message).replace(/\n/g, "<br />");
  const subject = `New portfolio message from ${notification.name}`;
  const text = [
    `New portfolio inquiry from ${notification.name} (${notification.email}):`,
    `Company: ${notification.company || "Not specified"}`,
    "",
    `Message:`,
